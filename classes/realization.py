...
class DigitalFilterRealization(QMainWindow):
    ...
    def draw_directform_2(self):
        ...
        self.add_line(50, (1+1)*delay_distance-25,delay_distance+25, (1+1)*delay_distance-25)
        input_text.setDefaultTextColor(Qt.white)
        
        
        output_text = self.scene.addText("y[n]")
        output_text.setPos(2*delay_distance+50, 160)
        output_text.setDefaultTextColor(Qt.white)

        # Connect to output
        self.add_line(2*delay_distance+25, (1+1)*delay_distance-25, 2*delay_distance+50, (1+1)*delay_distance-25)
        
        for i in range(1, max(len(self.zeros),len(self.poles))+1):
            if i == len(self.zeros):
                self.add_line(delay_distance+25, (i+1)*delay_distance-25,delay_distance+25, (i+2)*delay_distance-40)  ## vertical line connecting delays
            else:                 
                self.add_line(delay_distance+25, (i+1)*delay_distance-25,delay_distance+25, (i+2)*delay_distance-25)  ## vertical line c
            delay = self.add_block(delay_distance, (i+1)*delay_distance, f"z⁻{i}") ##delay blocks
            self.x_delays.append(delay)
            # self.add_line(0.5*delay_distance-5,(i+1)*100-25,delay_distance+25,(i+1)*100-25)
            
        for i in range(1,len(self.zeros)+1):
            self.add_line(1.5*delay_distance+40, (i+1)*100-10,1.5*delay_distance+40, (i+2)*100-40) #vertical lines connecting summation
            self.add_line(delay_distance+25,(i+1)*100-25,1.5*delay_distance+25,(i+1)*100-25,  text = f"{self.zeros[i-1]}")
            self.add_summation_circle(1.5*delay_distance+25, (i+1)*100-40)

        self.add_line(delay_distance+25,(i+2)*100-40, 1.5*delay_distance+40, (i+2)*100-40, text = f"{self.zeros[i-1]}")


        for i in range(1,len(self.poles)+1):
            self.add_line(0.5*delay_distance-5,(i+1)*100-25,delay_distance+25,(i+1)*100-25,text = f"{self.poles[i-1]}")
        ...
        self.add_line(0.5*delay_distance+10,(i+2)*100-25,delay_distance+25, (i+2)*100-25,text = f"{self.poles[i-1]}")
    ...
    def draw_direct_form_1(self):
        ...
        # Output
        output_text = self.scene.addText("y[n]")
        output_text.setPos(450, 160)
        output_text.setDefaultTextColor(Qt.white)

        # Connect to output
        self.add_line(400+25, (1+1)*100-25, 450, (1+1)*100-25)
        for i in range(1, len(self.zeros)+1):
        # First Delay Block (z^-1)
            ...
            self.add_line(100+25, (i+1)*100-25,200+25, (i+1)*100-25, text = f"b{i-1}") ## horizontal line connecting z and sum
            self.add_summation_circle(200+25, (i+1)*100-40)
            self.add_line(200+40, (i+1)*100-10,200+40, (i+2)*100-40) # vertical lines connecting summation

            x_delay = self.add_block(100, (i+1)*100, f"z⁻{i}")
          
            self.x_delays.append(x_delay)
        self.add_line(100+25,(i+2)*100-40, 200+40, (i+2)*100-40, f"b{i-1}")
        self.add_line(200+55, 200-25,300+25, 200-25)
        ...
        # First Delay Block (z^-1)
            ...
            self.add_line(300+25, (i+1)*100-25,400+25, (i+1)*100-25, text = f"a{i-1}")
            self.add_summation_circle(300-5, (i+1)*100-40)
            self.add_line(300+10, (i+1)*100-10,300+10, (i+2)*100-40)

            y_delay = self.add_block(400, (i+1)*100, f"z⁻{i}")
          
            self.y_delays.append(y_delay)
        self.add_line(300+10,(i+2)*100-40, 400+25, (i+2)*100-40, f"a{i-1}")
    ...
    def add_summation_circle(self, x, y):
        ...
        summation = QGraphicsEllipseItem(x, y, 30, 30)
        summation.setPen(QPen(Qt.white, 2))
